...
Template.registerHelper 'renderNodeLeaves', -> !Session.get("apiTreeCollapse") and @._id is Session.get("currentApiNode")?._id and Model.ApiMachineLeaf.find({parent: Session.get('currentApiNode')?._id}).count() > 0
