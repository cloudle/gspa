...
Meteor.methods
  insertBranchProduct: (productId = null, branchId = null)->
    ...
    return {valid: false, error: 'branch no found!'} if !(branch = Schema.Branch.findOne(branchId))
  ...
  removeBranchProduct: (branchProductId)->
    branchProduct = Schema.BranchProduct.findOne({_id: branchProductId, allowDelete: true})
    return {valid: false, error: 'branchProductId is not valid!'} if !branchProduct
